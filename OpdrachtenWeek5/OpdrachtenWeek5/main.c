...
void displayDriverInit() 
{
	spi_slaveSelect(0);				// Select display chip (MAX7219)
  	...
  	spi_write(0x0F);    			//  -> Level 4 (in range [1..F]) // Done : changed it to set it to highest for exercise B2
  	spi_slaveDeSelect(0);			// Deselect display chip

  	spi_slaveSelect(0);				// Select display chip
  	spi_write(0x0B);  				// Register 0B: Scan-limit
  	spi_write(0x03);   				// 	-> 1 = Display digits 0..1 // Done: changed it to 0x03 so it can control 4 digets instead of one.
  	spi_slaveDeSelect(0);			// Deselect display chip
